...
c      
	...
	maxbumps = 20000
      ...
      read(stdin,*) tol
      write(stdout,*)'Bandpass filter (f_low, f_high, n_but,i_pass)?'
      read(stdin,*) f_low,f_high,n_but,i_pass
...
c
c      call gfilter(f,gwidth,npts,dt)
c      call gfilter(g,gwidth,npts,dt)
      call wsac1('numerator1',f,npts,beg,dt,nerr)
      call bandpassfilter(f,npts,dt,n_but,i_pass,f_low,f_high)
      call wsac1('numerator',f,npts,beg,dt,nerr)
      call wsac1('denominator1',g,npts,beg,dt,nerr)
      call bandpassfilter(g,npts,dt,n_but,i_pass,f_low,f_high)
...
c 
      ...
      write(ccfile,'(a1,i5.5)') 'c',0
      call wsac1(ccfile,g,npts,beg,dt,nerr)
c
...
c
      call zero(p,MAXPTS)
      call build_decon(amps,shifts,nshifts,p,npts,
     &       n_but,i_pass,f_low,f_high,dt)
      if(verbose) then
...
c
     ...
     &       n_but,i_pass,f_low,f_high,dt)
	if(verbose)then
          write(filename,'(a1,i5.5)') 'd',nshifts
          call phs_shift(p,theshift,npts,dt)      
...
c        
	call zero(g,MAXPTS)
        ...
        call convolve(p,g,npts,dt)
	if(verbose)then
          write(filename,'(a1,i5.5)') 'p',nshifts
...
c        call gfilter(f,gwidth,npts,dt)
        ...
        sumsq_ip1 = sumsq_ip1/ power
        write(resfile,'(a1,i5.5)') 'r',nshifts
	if(verbose)then
          call wsac1(resfile,r,npts,beg,dt,nerr)
	end if
	d_error = 100*(sumsq_i - sumsq_ip1)
...
c        call gfilter(p,gwidth,npts,dt)
        call bandpassfilter(p,npts,dt,n_but,i_pass,f_low,f_high)
...
*
      ...
      real f(MAXPTS), g(MAXPTS), c(MAXPTS)
      real sum0, temp
      integer i,n,n2,n2o2
c
...
************************************************************************
*
      subroutine build_decon(amps,shifts,nshifts,p,n,
     &           n_but,i_pass,f_low,f_high,dt)
...
c      call gfilter(p,gwidth,n,dt)
      call bandpassfilter(p,n,dt,n_but,i_pass,f_low,f_high)
